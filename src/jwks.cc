...
class KeyGetter : public WithStatus {
 public:
  bssl::UniquePtr<EVP_PKEY> createEvpPkeyFromPem(const std::string& pkey_pem) {
    ...
    if (key == nullptr) {
      updateStatus(Status::PemParseError);
      return nullptr;
    }
    return key;
  }
...
void Jwks::addKeyFromPem(const std::string& pkey, const std::string& kid,
...
JwksPtr Jwks::createFromPem(const std::string& pkey, const std::string& kid,
  ...
  if (ret->keys_.size() != 1) {
    ret->updateStatus(Status::PemParseError);
    return ret;
  }
